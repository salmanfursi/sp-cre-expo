...
import { useGetDepartmentByIdQuery } from "../../../redux/conversation/conversationApi";

const ConversationFilter = ({data, setFilters, filters, availableFilters }) => {
  ...
  return (
    <View className="p-2 border-b border-gray-200 ">
      ...
      <View className="flex-row items-center justify-between">
        <View className="flex-row p-1 rounded-md bg-gray-200">
          <TouchableOpacity
            className={`px-4 py-2 rounded-lg ${
              filters?.messagesSeen === null ? "" : "bg-gray-400"
            }`}
            onPress={() =>
              setFilters({
                ...filters,
          ...
          >
            <Text
              className={`text-sm font-semibold ${
                filters?.messagesSeen === null ? "" : "text-gray-700"
              }`}
            >
              All
            </Text>
          ...
          <TouchableOpacity
            className={`px-4 py-2 rounded-lg ${
              filters?.messagesSeen === false ? "" : "bg-gray-400"
            }`}
            onPress={() =>
              setFilters({
                ...filters,
          ...
          >
            <Text
              className={`text-sm font-semibold ${
                filters?.messagesSeen === false ? " " : "text-gray-700"
              }`}
            >
              Unseen
            </Text>
      ...
      {isDropdownVisible && (
        <View className="p-2 bg-white rounded-lg shadow">
          ...
          <TouchableOpacity
            className="w-full bg-cyan-600 p-3 rounded-lg mt-2"
